...
def _backfill_batch(VideoTranscript, db_alias, old_code, new_code, batch):
    """
    Rename one batch of rows, skipping any video where new_code is already taken.

    Such a pair is the duplicate this migration exists to remove, but resolving it means
    deleting one of the two transcripts. That call is left to a human; the video is logged.
    """
    ...
    for video_id, provider, language_code in candidates:
        ...
        logger.warning(
            "0006_backfill_language_code_gcp_to_edx: not renaming %r -> %r for video %s; a %s "
            "transcript already holds %r. Both rows left in place; resolve manually.",
            old_code, new_code, video_id, provider, new_code,
        )
...
def edx_to_gcp_language_codes(apps, schema_editor):
    ...
    Lossy for Spanish, and for any row that legitimately held an edX code before this
    migration ran: "es-419" goes back to "es", while "es-es" was never produced by the
    forward mapping and is left alone.
    """
    VideoTranscript = apps.get_model("edxval", "VideoTranscript")
    db_alias = schema_editor.connection.alias
